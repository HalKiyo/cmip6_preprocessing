...
def main():
    ...
    first_begin, first_end = 1850, 1875
    middle_begin, middle_end = 1976, 1990
    # if last_flag is False, edit like last_begin=1, last_end=0
    last_begin, last_end = 1991, 2014
    # if loop_flag is False, edit like start_loop=1, stop_loop=0 and timestep_loop > 0
    start_loop, stop_loop = 1876, 1975
    timestep_loop = 25

    # load preprocess class
    pre = Preprocess(model, variable, timestep_loop,
                     first_begin, first_end, middle_begin, middle_end,
                     last_begin, last_end, start_loop, stop_loop, depth)
...
class Preprocess():
    def __init__(self, model, variable, timestep_loop, 
        ...
        self.middle_begin, self.middle_end = middle_begin, middle_end
        ...
        self.middle_data_num = 12*(self.middle_end - self.middle_begin + 1)
        self.last_data_num = 12*(self.last_end - self.last_begin + 1)
        self.loopyr = (self.stop_loop - self.start_loop + 1)/self.timestep_loop
        self.tm = int(self.data_num*self.loopyr + self.first_data_num + self.middle_data_num + self.last_data_num) # number of all data
        ...
        self.middle_file = f"{self.datadir}/{self.variable}_{self.middle_begin}-{self.middle_end}.nc"
    ...
    def make_val(self, conc_flag, first_flag=True, loop_flag=True, last_flag=True):
        ...
        # middle extra file
        val[-(self.middle_data_num+self.last_data_num):-self.last_data_num, :, :] = self._fill(
                self._load_val(self.middle_file, self.middle_data_num, self.depth, conc_flag))
        print(f"{self.middle_begin}-{self.middle_end} loaded")
