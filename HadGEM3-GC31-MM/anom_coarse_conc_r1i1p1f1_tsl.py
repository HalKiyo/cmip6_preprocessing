...
def main():
    ...
    first_begin, first_end = 1850, 1869
    # if last_flag is False, edit like last_begin=1, last_end=0
    last_begin, last_end = 2010, 2014
    # if loop_flag is False, edit like start_loop=1, stop_loop=0 and timestep_loop > 0
    start_loop, stop_loop = 1870, 1969
    timestep_loop = 20
    extra1_begin, extra1_end = 1970, 1987
    extra2_begin, extra2_end = 1988, 1989
    extra3_begin, extra3_end = 1990, 2009
    ...
    pre = Preprocess(model, variable, timestep_loop,
                     ...
                     extra1_begin, extra1_end, extra2_begin, extra2_end, extra3_begin, extra3_end,
                     depth)
...
class Preprocess():
    def __init__(self, model, variable, timestep_loop, 
        ...
        self.extra1_begin, self.extra1_end = extra1_begin, extra1_end
        self.extra2_begin, self.extra2_end = extra2_begin, extra2_end
        self.extra3_begin, self.extra3_end = extra3_begin, extra3_end
        ...
        self.extra1_data_num = 12*(self.extra1_end - self.extra1_begin + 1)
        self.extra2_data_num = 12*(self.extra2_end - self.extra2_begin + 1)
        self.extra3_data_num = 12*(self.extra3_end - self.extra3_begin + 1)
        self.last_data_num = 12*(self.last_end - self.last_begin + 1)
        self.loopyr = (self.stop_loop - self.start_loop + 1)/self.timestep_loop
        self.tm = int(self.data_num*self.loopyr + self.first_data_num + self.last_data_num + 
                      self.extra1_data_num + self.extra2_data_num + self.extra3_data_num) # number of all data
        ...
        self.extra1_file = f"{self.datadir}/{self.variable}_{self.extra1_begin}-{self.extra1_end}.nc"
        self.extra2_file = f"{self.datadir}/{self.variable}_{self.extra2_begin}-{self.extra2_end}.nc"
        self.extra3_file = f"{self.datadir}/{self.variable}_{self.extra3_begin}-{self.extra3_end}.nc"
    ...
    def make_val(self, conc_flag, first_flag=True, loop_flag=True, last_flag=True):
        ...
        # extra files
        ind1 = self.extra1_data_num + self.extra2_data_num + self.extra3_data_num + self.last_data_num
        val[-ind1:-ind1+self.extra1_data_num, :, :] = self._fill(
                self._load_val(self.extra1_file, self.extra1_data_num, self.depth, conc_flag))
        print(f"{self.extra1_begin}-{self.extra1_end} loaded")

        ind2 = self.extra2_data_num + self.extra3_data_num + self.last_data_num
        val[-ind2:-ind2+self.extra2_data_num :, :] = self._fill(
                self._load_val(self.extra2_file, self.extra2_data_num, self.depth, conc_flag))
        print(f"{self.extra2_begin}-{self.extra2_end} loaded")

        ind3 = self.extra3_data_num + self.last_data_num
        val[-ind3:-ind3+self.extra3_data_num, :, :] = self._fill(
                self._load_val(self.extra3_file, self.extra3_data_num, self.depth, conc_flag))
        print(f"{self.extra3_begin}-{self.extra3_end} loaded")
    ...
    def interpolation(self, val_anom):
        val_coarse = val_anom[:, :int(self.lt/self.upscale_rate), :int(self.ln/self.upscale_rate)]
        for time in range(len(val_coarse)):
            val_coarse[time, :, :] = basemap.interp(val_anom[time, :, :],
                                                    self.lons, self.lats, 
